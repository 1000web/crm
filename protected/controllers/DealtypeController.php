...
class DealtypeController extends Controller
{
    ...
    public function actionCreate($copy = NULL)
    {
        ...
        if (isset($_POST['DealType'])) {
            $this->_model->attributes = $_POST['DealType'];
            if ($this->_model->save()) {
                $log = new DealTypeLog;
    ...
    public function actionIndex()
    {
        ...
        $this->_pagesize = $userProfile->deal_type_pagesize;
        $this->buildPageOptions();

        $this->_filter = new DealType('search');
        $this->_filter->unsetAttributes(); // clear any default values
        if (isset($_GET['DealType'])) $this->_filter->attributes = $_GET['DealType'];

        $this->render('index', array(
            'dataProvider' => DealType::model()->getAll($userProfile),
        ));
    }
    ...
    public function actionLog($id)
    {
        ...
        $this->render('log', array(
            'dataProvider' => DealTypeLog::model()->getLog($id, $this->_pagesize),
        ));
    }
    ...
    public function loadModel($id = NULL)
    {
        ...
        if ($this->_model === NULL) {
            $this->_model = DealType::model()->findbyPk($id);
     ...
     * @param DealType $model the model to be validated
     */
    protected function performAjaxValidation($model)
    {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'deal-type-form') {
