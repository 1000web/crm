...
class ShippingController extends Controller
{
    ...
    {
        $this->_model = new Shipping;

        // Uncomment the following line if AJAX validation is needed
        // $this->performAjaxValidation($model);

        if (isset($_POST['Shipping'])) {
            $this->_model->attributes = $_POST['Shipping'];
            if ($this->_model->save()) {
                $log = new ShippingLog;
    ...
    {
        $this->_model = new Shipping('search');
        $this->_model->unsetAttributes(); // clear any default values
        if (isset($_GET['Shipping']))
            $this->_model->attributes = $_GET['Shipping'];
    ...
    public function loadModel($id = NULL)
    {
        ...
        if ($this->_model === NULL) {
            $this->_model = Shipping::model()->findbyPk($id);
