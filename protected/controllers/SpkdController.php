...
class SpkdController extends Controller
{
    /**
     * Creates a new model.
    ...
    public function actionCreate($copy = NULL)
    {
        ...
        if (isset($_POST['Spkd'])) {
            $this->_model->attributes = $_POST['Spkd'];
            if ($this->_model->save()) {
                $log = new SpkdLog;
    ...
    public function actionIndex()
    {
        ...
        $this->_pagesize = $userProfile->spkd_pagesize;
        $this->buildPageOptions();

        $this->_filter = new Spkd('search');
        $this->_filter->unsetAttributes(); // clear any default values
        if (isset($_GET['Spkd'])) $this->_filter->attributes = $_GET['Spkd'];

        $this->render('index', array(
            'dataProvider' => Spkd::model()->getAll($userProfile),
        ));
    }
    ...
    public function actionLog($id)
    {
        ...
        $this->render('log', array(
            'dataProvider' => SpkdLog::model()->getLog($id, $this->_pagesize),
        ));
    }
    ...
    public function loadModel($id = NULL)
    {
        ...
        if ($this->_model === NULL) {
            $this->_model = Spkd::model()->findbyPk($id);
     ...
     * @param Spkd $model the model to be validated
     */
